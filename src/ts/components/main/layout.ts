const svgWidth = 2000;
const svgHeigth = 2000;
const xArr = [-400, -100, -325];
const yArr = [10, 140, -405];
const lineBreaks = [210, -90, 500];
const lineLenght = [275, 240, 310];
const xGap = 435;
const yGap = 360;
...
for (let i = 0; i < xArr.length; i += 1) {
  ...
  for (let count = 0; y < svgHeigth; y += yGap, count += 1) {
    ...
    for (x -= count * 20; x < svgWidth + lineLenght[i]; x += xGap) {
      const line1 = `<line x1="${x}" y1="${y}" x2="${
        x + lineLenght[i]
      }" y2="${y}" transform="rotate(-45)" />`;
      const line2 = `<line x1="${x + lineLenght[i]}" y1="${y + lineBreaks[i]}" x2="${x}" y2="${
        y + lineBreaks[i]
      }"  transform="rotate(-45)" />`;
